...
public class AdapterTeachTeacher extends RecyclerView.Adapter<AdapterTeachTeacher.Holder>
{
	private ActivityTeachTeacher owner = null;
	private List<STeachLine> teachLines = null;

	public AdapterTeachTeacher(ActivityTeachTeacher owner)
	{
		this.owner = owner;
	}
	...
	{
		return new Holder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_teach_teacher, parent, false));
	}
	...
	public void onBindViewHolder(final Holder holder, int position)
	{
		...
		holder.name.setText(String.format("%02d月%02d日", teachLine.month, teachLine.day));
		...
		holder.touch.setOnTouchListener(new View.OnTouchListener()
		{
			...
			public boolean onTouch(View view, MotionEvent event)
			{
				...
				int count = ActivityTeachTeacher.blockEnd - ActivityTeachTeacher.blockBegin;
					...
					if (targetFlag != -1)
					{
						...
						for (int i = 0; i < holder.eventCon.getChildCount(); i++)
						{
							...
							if (i >= min && i <= max && canModify)
							{
								...
								teach.teacherId = owner.teacher.id;
								teach.year = teachLine.year;
								teach.month = teachLine.month;
								teach.day = teachLine.day;
