...
public class DialogSelectFriendToSendTask extends Dialog implements View.OnClickListener
{
	public interface OnListener
	{
		void onSelect(List<SUser> selectedList, String taskText);
	}
	...
	private DialogSelectFriendToSendTask(@NonNull Context context, String userId)
	{
		super(context, R.style.FullDialog);
//		setCancelable(false);
		setContentView(R.layout.dialog_select_friend_to_send_task);
		initHolder();
		...
		case R.id.ok:
			if (adapter.isReady())
			{
				if (adapter.getSelectedUsers().size() > 0)
				{
					dismiss();
					if (listener != null)
						listener.onSelect(adapter.getSelectedUsers(), holder.editText.getText().toString());
				}
				else
				{
	...
	{
		DialogSelectFriendToSendTask dialog = new DialogSelectFriendToSendTask(context, userId);
