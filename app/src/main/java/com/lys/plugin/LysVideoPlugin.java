...
public class LysVideoPlugin implements IPluginModule, IPluginRequestPermissionResultCallback
{
	...
	public void onClick(Fragment fragment, RongExtension extension)
	{
		...
		String[] permissions = new String[]{Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.RECORD_AUDIO, Manifest.permission.CAMERA};
				...
				new Handler().postDelayed(new Runnable()
				{
					...
					public void run()
					{
						try
						{
							...
							linkedHashMap.put("file://" + mFilepath, 3);
	...
	private void start(Fragment fragment, RongExtension extension)
	{
		try
		{
			...
			extension.startActivityForPluginResult(intent, REQUEST_CODE_VIDEO, this);
		}
		catch (Exception e)
		{
			e.printStackTrace();
